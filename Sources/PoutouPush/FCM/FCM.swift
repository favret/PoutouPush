...
*/
public class FCM {
  private let server = "https://fcm.googleapis.com/fcm/send"
  ...
  private func build(body: FCMPayload) -> UnsafeMutablePointer<Int8>? {
  ...
  public func push(title: String, text: String, badge: Int? = nil, sound: String? = nil, extra: [String: String]? = nil, to token: String) {
    let body = FCMPayloadBody(title: title, text: text, badge: badge, sound: sound, extra: extra)
    let payload = FCMPayload(body: body, to: token)

    self.push(payload: payload)
  }

  public func push(payload: FCMPayload) {
